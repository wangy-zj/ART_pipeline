...
int main(int argc, char *argv[]){  
 ...
 // 读取解析各项输入参数 
  ...
  //int beamform_dbuf_size = input_dbuf_size*Beams*bf_nbit/naverage_bf; //beamform ringbuffer
  //int zoom_dbuf_size = Beams*zoom_nchan*zoom_nsamp*bf_nchan*zoom_nbit/naverage_zoom; //zoom fft ringbuffer
  unsigned input_dbuf_size  = ipcbuf_get_bufsz(input_dblock);
  unsigned beamform_dbuf_size = ipcbuf_get_bufsz(beamform_output_dblock);
  unsigned zoom_dbuf_size = ipcbuf_get_bufsz(zoom_output_dblock);
  ...
  if (ascii_header_set(beamform_hbuf, "HDR_SIZE", "%d", 4096) < 0)  {
    fprintf(stderr, "BEAMFORM_ERROR: Error setting HDR_SIZE, "
